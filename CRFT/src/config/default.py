...
##############  ↓  CRFT Pipeline  ↓  ##############
_CN.CRFT = CN()
_CN.CRFT.RESOLUTION = (8, 2)  # options: [(8, 2)]
_CN.CRFT.FINE_WINDOW_SIZE = 5  # window_size in fine_level, must be odd
_CN.CRFT.MEDIUM_WINDOW_SIZE = 3  # window_size in fine_level, must be odd

# 1. CRFT-backbone (local feature CNN) config
_CN.CRFT.RESNET = CN()
_CN.CRFT.RESNET.INITIAL_DIM = 128
_CN.CRFT.RESNET.BLOCK_DIMS = [128, 196, 256]  # s1, s2, s3

# 2. CRFT-coarse module config
_CN.CRFT.COARSE = CN()
_CN.CRFT.COARSE.INFERENCE = INFERENCE
_CN.CRFT.COARSE.D_MODEL = 256
_CN.CRFT.COARSE.D_FFN = 256
_CN.CRFT.COARSE.NHEAD = 8
_CN.CRFT.COARSE.LAYER_NAMES = ['self', 'cross'] * 4
_CN.CRFT.COARSE.ATTENTION = 'linear'  # options: ['linear', 'full']


# 3. Coarse-Matching config
_CN.CRFT.MATCH_COARSE = CN()
_CN.CRFT.MATCH_COARSE.INFERENCE = INFERENCE
_CN.CRFT.MATCH_COARSE.D_MODEL = 256
_CN.CRFT.MATCH_COARSE.THR = 0.3
_CN.CRFT.MATCH_COARSE.BORDER_RM = 2
_CN.CRFT.MATCH_COARSE.MATCH_TYPE = 'dual_softmax'  # options: ['dual_softmax']
_CN.CRFT.MATCH_COARSE.DSMAX_TEMPERATURE = 0.1
_CN.CRFT.MATCH_COARSE.TRAIN_COARSE_PERCENT = 0.2  # training tricks: save GPU memory
_CN.CRFT.MATCH_COARSE.TRAIN_PAD_NUM_GT_MIN = 200  # training tricks: avoid DDP deadlock

# 4. CRFT-fine module config
_CN.CRFT.FINE = CN()
_CN.CRFT.FINE.DENSER = False # if true, match all features in fine-level windows
_CN.CRFT.FINE.INFERENCE = INFERENCE
_CN.CRFT.FINE.DSMAX_TEMPERATURE = 0.1
_CN.CRFT.FINE.THR = 0.1
_CN.CRFT.FINE.MLP_HIDDEN_DIM_COEF = 2 # coef for  mlp hidden dim (hidden_dim = feat_dim * coef)
_CN.CRFT.FINE.NHEAD_FINE_LEVEL = 8
_CN.CRFT.FINE.NHEAD_MEDIUM_LEVEL = 7


# 5. CRFT Losses

_CN.CRFT.LOSS = CN()
_CN.CRFT.LOSS.FOCAL_ALPHA = 0.25
_CN.CRFT.LOSS.FOCAL_GAMMA = 2.0
_CN.CRFT.LOSS.POS_WEIGHT = 1.0
_CN.CRFT.LOSS.NEG_WEIGHT = 1.0

# -- # coarse-level
_CN.CRFT.LOSS.COARSE_WEIGHT = 0.5
# -- # fine-level
_CN.CRFT.LOSS.FINE_WEIGHT = 0.3
# -- # sub-pixel
_CN.CRFT.LOSS.SUB_WEIGHT = 1 * 10**4
...
# OSDataset options
_CN.DATASET.OS_IMG_RESIZE =64  # resize the longer side, zero-pad bottom-right to square.
_CN.DATASET.OS_IMG_PAD = True  # pad img to square with size = OS_IMG_RESIZE
_CN.DATASET.OS_DF = 8
...
_CN.TRAINER.SB_SUBSET_SAMPLE_REPLACEMENT = True  # whether sample each scene with replacement or not 是否允许同一个样本在一个epoch中被多次采样
_CN.TRAINER.SB_SUBSET_SHUFFLE = True  # after sampling from scenes, whether shuffle within the epoch or not 是否在每个epoch内部再打乱
_CN.TRAINER.SB_REPEAT = 1  # repeat N times for training the sampled data 每个epoch对同一个样本采样几次
# 'random' config
...
_CN.CRFT.PRETRAIN_PATCH_SIZE = _CN.PRETRAIN.PATCH_SIZE 
...
def get_cfg_defaults(inference=False):
    ...
    if inference:
        _CN.CRFT.COARSE.INFERENCE = True
        _CN.CRFT.MATCH_COARSE.INFERENCE = True
        _CN.CRFT.FINE.INFERENCE = True
    return _CN.clone()
